...
const hrd_1 = require("./hrd");
const sps_1 = require("./sps");
function get_seq_parameter_set_svc_ext(sps, stream, flag) {
    const svc = {
...
function get_svc_vui_parameters_ext(stream, flag) {
    if (!flag)
        return [];
    ...
    for (let i = 0; i <= num_entries_minus_1; i++) {
        ...
        if (svc.nal_hrd_parameters_present_flag) {
            hrd_1.hrd_parameters(svc.nal_hrd_parameters, stream);
        }
        svc.vcl_hrd_parameters_present_flag = stream.readBit();
        if (svc.vcl_hrd_parameters_present_flag) {
            hrd_1.hrd_parameters(svc.vcl_hrd_parameters, stream);
        }
        if (svc.nal_hrd_parameters_present_flag ||
...
function get_mvc_vui_parameters_ext(stream, flag) {
    if (!flag)
        return [];
    ...
    for (let i = 0; i <= num_ops_minus_1; i++) {
        ...
        if (mvc.nal_hrd_parameters_present_flag) {
            hrd_1.hrd_parameters(mvc.nal_hrd_parameters, stream);
        }
        mvc.vcl_hrd_parameters_present_flag = stream.readBit();
        if (mvc.vcl_hrd_parameters_present_flag) {
            hrd_1.hrd_parameters(mvc.vcl_hrd_parameters, stream);
        }
        if (mvc.nal_hrd_parameters_present_flag ||
...
function get_seq_parameter_set_mvc_ext(sps, stream, flag) {
    const mvc = {
        views: [],
        levels: [],
        mfc_format_idc: 0,
    ...
    for (let i = 0; i < num_levels; i++) {
        ...
        const applicable_op_temporal_id = [];
        const applicable_op_num_target_views = [];
        const applicable_op_target_view_id = [];
        const applicable_op_num_views = [];
...
function get_mvcd_vui_parameters_ext(stream) {
    const ops = [];
    ...
    for (let i = 0; i <= num_ops_minus1; i++) {
        ...
        if (op.nal_hrd_parameters_present_flag) {
            hrd_1.hrd_parameters(op.nal_hrd_parameters, stream);
        }
        op.vcl_hrd_parameters_present_flag = stream.readBit();
        if (op.vcl_hrd_parameters_present_flag) {
            hrd_1.hrd_parameters(op.vcl_hrd_parameters, stream);
        }
        if (op.nal_hrd_parameters_present_flag ||
...
function get_seq_parameter_set_mvcd_ext(stream, flag) {
    const mvcd = {
        num_views: 0,
        views: [],
        num_depth_views: 0,
    ...
    for (let i = 1; i <= num_views_minus1; i++) {
        ...
        if (!view.depth_view_present_flag) {
            continue;
        }
        const num_anchor_refs_l0 = stream.ExpGolomb();
        for (let j = 0; j < num_anchor_refs_l0; j++) {
...
*/
function get_seq_parameter_set_3davc_ext(_stream, _flag, _mvcd) {
    const avc = {
        view_id_3dv: [],
    ...
    switch (sps.profile_idc) {
        ...
        case 86: {
            ...
            ssps.seq_parameter_set_3davc_ext =
                get_seq_parameter_set_3davc_ext(stream, 0, ssps.seq_parameter_set_mvcd_ext);
            break;
        }
        case 118:
        case 128:
        ...
        case 139: {
            ...
            ssps.seq_parameter_set_mvcd_ext =
                get_seq_parameter_set_mvcd_ext(stream, 1);
            ssps.seq_parameter_set_3davc_ext =
                get_seq_parameter_set_3davc_ext(stream, 1, ssps.seq_parameter_set_mvcd_ext);
            ssps.seq_parameter_set_mvc_ext =
                get_seq_parameter_set_mvc_ext(sps, stream, 0);
            ssps.mvc_vui_parameters_ext =
                get_mvc_vui_parameters_ext(stream, 0);
